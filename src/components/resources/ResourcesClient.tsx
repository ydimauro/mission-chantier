  ...
  return <div className="mx-auto max-w-6xl px-4 py-10">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[15rem_1fr]">
      ...
      <section aria-labelledby="resources-list-title">
        ...
        <ul className="mt-3 grid gap-3 sm:grid-cols-2">{entries.map((entry) => <li key={entry.id} className="rounded-md border border-border bg-surface p-4">{entry.category === "Glossaire" ? null : <ResourcePhoto resourceId={entry.id} term={entry.term}/>}<p className={entry.category === "Glossaire" ? "text-xs font-bold uppercase tracking-wide text-ink-muted" : "mt-3 text-xs font-bold uppercase tracking-wide text-ink-muted"}>{entry.category} · {entry.subcategory}</p><h3 className="mt-1 text-lg font-bold text-ink">{entry.term}</h3><p className="mt-2 text-sm leading-6 text-ink">{entry.description}</p><p className="mt-3 text-xs text-ink-muted">Utilisé dans : {entry.missions.join(", ")}</p></li>)}</ul>
