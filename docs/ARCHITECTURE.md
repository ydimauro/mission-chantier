...
>
> **État réel au terme de l’ÉTAPE 20** (Playwright) : cinq nouveaux fichiers dans `test/e2e/` (`student-5e-journey`, `student-4e-journey`, `teacher-workflow`, `backup-and-conflict`, `rgpd-erase`), en plus du test existant depuis l’ÉTAPE 10. Le parcours professeur reconstruit un `.mctkey` minimal directement en mémoire dans le test (jamais versionné) pour importer un vrai fichier élève exporté par l’interface et vérifier le retour des résultats corrigés. Le repli de sauvegarde Firefox est vérifié en simulant l’absence de `window.showDirectoryPicker` sur Chromium (Firefox natif indisponible dans l’environnement d’exécution, docs/rapports/ETAPE_16.md § 7). Détail complet dans `docs/rapports/ETAPE_20.md`.
