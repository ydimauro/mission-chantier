...
>
> **État réel au terme de l’ÉTAPE 20** (Playwright) : huit scénarios couvrent les parcours élève 5e et 4e, l’espace professeur, les sauvegardes Chromium et le repli Firefox, les conflits de versions, le mauvais fichier et l’effacement RGPD. Le parcours professeur reconstruit un `.mctkey` minimal en mémoire, corrige un vrai fichier élève, l’exporte puis le réimporte dans `/progression`. La page élève affiche alors la note et les compétences évaluées grâce à `src/lib/evaluations/student-summary.ts`. Le repli Firefox est vérifié en simulant l’absence de `window.showDirectoryPicker` sur Chromium. Détail complet dans `docs/rapports/ETAPE_20.md`.
